...
//MARK: - Actions
    ...
    func clickSignUpButton(_ sender: UIButton) {
        updateSignUp()
        let model: userModel = userModel(name: "", email: emailContainer.textField.text ?? "", password: passwordContainer.textField.text ?? "", grade: 0, class: 0, s_number: 0, uid: "\(UUID.init())")
